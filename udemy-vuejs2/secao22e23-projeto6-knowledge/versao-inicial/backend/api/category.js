module.exports = app => {
  const { existsOrError, notExistsOrError, lessEqualOrError } = app.api.validation;
  ...
  const save = (req, res) => {
    ...
    try {
      ...
      lessEqualOrError(category.name, "O nome da categoria deve possuir, no máximo, 255 caracteres", 255);
    } catch (msg) {
